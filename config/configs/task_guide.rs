...
pub struct TaskGuideTable {
    records: Vec<TaskGuide>,
...
impl TaskGuideTable {
    pub fn load(path: &str) -> anyhow::Result<Self> {
        ...
        let records: Vec<TaskGuide> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&TaskGuide> {
        self.by_id.get(&id).map(|&i| &self.records[i])
    }

    #[inline]
    pub fn all(&self) -> &[TaskGuide] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, TaskGuide> {
        self.records.iter()
    }
