...
pub struct ChallengeTaskTable {
    records: Vec<ChallengeTask>,
    by_id: HashMap<i32, usize>,
    by_group: HashMap<i32, Vec<usize>>,
}

impl ChallengeTaskTable {
    pub fn load(path: &str) -> anyhow::Result<Self> {
        let json = std::fs::read_to_string(path)?;
        let value: serde_json::Value = serde_json::from_str(&json)?;

        let records: Vec<ChallengeTask> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&ChallengeTask> {
        self.by_id.get(&id).map(|&i| &self.records[i])
    }

    pub fn by_group(&self, group_id: i32) -> impl Iterator<Item = &'_ ChallengeTask> + '_ {
        self.by_group
            .get(&group_id)
            .into_iter()
    ...
    #[inline]
    pub fn all(&self) -> &[ChallengeTask] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, ChallengeTask> {
        self.records.iter()
    }
