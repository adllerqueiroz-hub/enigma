...
pub struct DiceTaskTable {
    records: Vec<DiceTask>,
...
impl DiceTaskTable {
    pub fn load(path: &str) -> anyhow::Result<Self> {
        ...
        let records: Vec<DiceTask> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&DiceTask> {
        self.by_id.get(&id).map(|&i| &self.records[i])
    }

    #[inline]
    pub fn all(&self) -> &[DiceTask] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, DiceTask> {
        self.records.iter()
    }
