...
pub struct OdysseyTaskTable {
    records: Vec<OdysseyTask>,
    by_id: HashMap<i32, usize>,
    by_group: HashMap<i32, Vec<usize>>,
}

impl OdysseyTaskTable {
    pub fn load(path: &str) -> anyhow::Result<Self> {
        let json = std::fs::read_to_string(path)?;
        let value: serde_json::Value = serde_json::from_str(&json)?;

        let records: Vec<OdysseyTask> = if let Some(array) = value.as_array() {
    ...
    #[inline]
    pub fn get(&self, id: i32) -> Option<&OdysseyTask> {
        self.by_id.get(&id).map(|&i| &self.records[i])
    }

    pub fn by_group(&self, group_id: i32) -> impl Iterator<Item = &'_ OdysseyTask> + '_ {
        self.by_group
            .get(&group_id)
            .into_iter()
    ...
    #[inline]
    pub fn all(&self) -> &[OdysseyTask] {
        &self.records
    }

    #[inline]
    pub fn iter(&self) -> std::slice::Iter<'_, OdysseyTask> {
        self.records.iter()
    }
