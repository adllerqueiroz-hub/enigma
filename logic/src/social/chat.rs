...
impl SocialManager {
    pub async fn send_msg(
        ...
        let (sender_name, level) = player_infos::get_user_basic_info(db, self.player_id)
            .await
            .map(|(name, level, _)| (name, level as u32))?;
        let portrait = player_infos::get_player_info(db, self.player_id)
            .await?
        ...
        Ok((
            ChatMsgPush {
                msg: vec![ChatMsg {
                    ...
                    sender_id: Some(self.player_id as u64),
    ...
    pub fn delete_offline_msg(&self) -> DeleteOfflineMsgReply {
        DeleteOfflineMsgReply {}
    }

    pub fn report(&self) -> ReportReply {
        ReportReply {}
    }

    pub fn word_test(&self) -> WordTestReply {
        WordTestReply {}
    }
}
