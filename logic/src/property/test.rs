...
async fn set_simple_property_persists_value() {
    ...
    set_simple_property(&pool, 7, 9, "on".to_string())
        .await
        .unwrap();
...
async fn main_scene_skin_requires_its_configured_item() {
    ...
    set_main_scene_skin(&pool, config::configs::get(), 8, 0)
        .await
        .unwrap();
    assert!(
        set_main_scene_skin(&pool, config::configs::get(), 8, 240002)
            .await
            .is_err()
    );
...
async fn main_ui_skin_requires_its_configured_item() {
    ...
    set_ui_style_skin(&pool, config::configs::get(), 9, 0)
        .await
        .unwrap();
    assert!(
        set_ui_style_skin(&pool, config::configs::get(), 9, 350002)
            .await
            .is_err()
    );
