...
impl RoomManager {
    pub async fn room_plan_info(
        ...
        refresh_current_room_plan(db, tables, self.player_id).await?;
        Ok(room_plan::get_room_plan_info(db, self.player_id).await?)
    }
    ...
    pub async fn room_plan_details(
        ...
        Ok(GetRoomPlanDetailsReply {
            info: room_plan::get_room_plan(db, self.player_id, plan_id).await?,
        })
    }
    ...
    pub async fn set_room_plan(
        ...
        let info =
            current_room_plan_snapshot(db, tables, self.player_id, plan_id, cover_id, name.clone())
                .await?;
        room_plan::save_room_plan(db, self.player_id, &info).await?;
    ...
    pub async fn set_room_plan_name(
        ...
        room_plan::update_room_plan_name(db, self.player_id, plan_id, &name).await?;
    ...
    pub async fn set_room_plan_cover(
        ...
        room_plan::update_room_plan_cover(db, self.player_id, plan_id, cover_id).await?;
    ...
    pub async fn delete_room_plan(
        ...
        room_plan::delete_room_plan(db, self.player_id, plan_id).await?;
    ...
    pub async fn switch_room_plan(
        ...
        if id_a != 0 && id_b != 0 {
            room_plan::switch_room_plans(db, self.player_id, id_a, id_b).await?;
        } else {
            let saved_id = match (id_a, id_b) {
                (0, id) | (id, 0) if id != 0 => id,
                _ => return Err(AppError::InvalidRequest),
            };
            let current = load_current_room_plan(db, tables, self.player_id).await?;
            let mut selected = room_plan::get_room_plan(db, self.player_id, saved_id)
                .await?
            ...
            selected.id = Some(0);
            room_plan::switch_active_room_plan(db, self.player_id, &previous, &mut selected)
                .await?;
        }
        Ok(SwitchRoomPlanReply {
            infos: room_plan::get_room_plan_info(db, self.player_id)
                .await?
                .infos,
        })
    }
    ...
    pub async fn use_room_plan(
        ...
        self.switch_room_plan(db, tables, 0, plan_id).await?;
    ...
    pub async fn share_room_plan(
        ...
        let share_code = format!("{}-{plan_id}", self.player_id);
        let can_share_count = room_plan::share_room_plan(db, self.player_id, plan_id, &share_code)
            .await?
            .ok_or(AppError::InvalidRequest)?;
    ...
    pub async fn use_room_share(
        ...
        let Some((share_user_id, mut info)) = room_plan::get_room_share(db, &share_code).await?
        else {
            return Err(AppError::InvalidRequest);
        };
        let source_plan_id = info.id.unwrap_or_default();
        prepare_copied_plan(&mut info, plan_id, cover_id, &name);
        let can_use_share_count = room_plan::save_copied_room_plan(
            db,
            self.player_id,
            share_user_id,
            source_plan_id,
    ...
    pub async fn copy_other_room_plan(
        ...
        let mut info = room_plan::get_room_plan(db, target_uid, plan_id)
            ...
            .ok_or(AppError::InvalidRequest)?;
        prepare_copied_plan(&mut info, plan_id, cover_id, &name);
        room_plan::save_room_plan_with_layout(db, self.player_id, &mut info).await?;
...
impl RoomManager {
    pub async fn delete_room_share(
        ...
        room_plan::set_share_code(db, self.player_id, plan_id, "").await?;
        Ok(sonettobuf::DeleteRoomShareReply { id: Some(plan_id) })
    }

    pub async fn room_log(&self, db: &SqlitePool) -> Result<GetRoomLogReply, AppError> {
        Ok(GetRoomLogReply {
            infos: room_plan::get_room_logs(db, self.player_id).await?,
        })
    }
}
