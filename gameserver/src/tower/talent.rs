...
pub async fn activate_talent(
    ...
    let talent = tables
        .tower_assist_talent(boss_id, talent_id)
    ...
    if talent.node_group != 0
        && tables.tower_assist_talents(boss_id).any(|row| {
            row.node_group == talent.node_group && plan.talent_ids.contains(&row.node_id)
        })
    {
        return Err(AppError::InvalidRequest);
    }
...
pub async fn reset_talent(
    ...
    let (reset_id, talent_point) = if talent_id == 0 {
        let total = tables
            .tower_assist_development(boss_id)
            .filter(|row| row.level <= plan.boss_level)
        ...
        let talent = tables
            ...
            .ok_or(AppError::InvalidRequest)?;
        let has_active_child = tables.tower_assist_talents(boss_id).any(|row| {
...
pub async fn change_talent_plan(
    ...
    let valid = (1..=custom_plan_count).contains(&plan_id)
        || tables.tower_talent_plan(boss_id, plan_id).is_some();
