...
pub async fn break_equip(
    ...
    let current = tables
        .equip_break_cost
        .iter()
        .find(|row| row.rare == equip_cfg.rare && row.break_level == target.break_lv)
        .ok_or(AppError::InvalidRequest)?;
    ...
    let Some(next) = tables
        .equip_break_cost
        .iter()
        .find(|row| row.rare == equip_cfg.rare && row.break_level == target.break_lv + 1)
    else {
        return Ok((EquipBreakReply {}, Vec::new(), Vec::new(), Vec::new()));
    };
