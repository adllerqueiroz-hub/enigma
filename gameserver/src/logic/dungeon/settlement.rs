...
pub async fn settle_battleless(
    ...
    let cost = reward::consume(
        &mut tx,
        player_id,
        &episode_cost(episode, completion.multiplier),
    )
    .await?;
...
async fn settle_completion_in_transaction(
    ...
    let rewards =
        reward::apply_dungeon_in_transaction(tx, player_id, completion_rewards.rewards).await?;
...
async fn settle_refund_rewards(
    ...
    let mut tx = db.begin().await?;
    let rewards = reward::apply_dungeon_in_transaction(&mut tx, player_id, refund).await?;
