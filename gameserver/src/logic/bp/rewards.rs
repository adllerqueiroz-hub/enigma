...
pub async fn get_bp_bonus(
    ...
    for bonus in config::configs::get().bp_lv_bonus.iter().filter(|bonus| {
        bonus.bp_id == bp_id
            && bonus.level <= max_claim_level
            && (claim_all || Some(bonus.level) == level)
    }) {
        if include_free
...
pub async fn get_self_select_bonus(
    ...
    let bonus = tables
        .bp_lv_bonus
        .iter()
        .find(|bonus| bonus.bp_id == bp_id && bonus.level == level)
        .ok_or(AppError::InvalidRequest)?;
    let selected =
