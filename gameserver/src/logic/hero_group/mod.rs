...
pub fn check_name(name: &str) -> Result<CheckHeroGroupNameReply, AppError> {
...
pub async fn update_group(
    ...
    if !hero_groups::update_hero_group(db, player_id, &saved).await? {
        return Err(AppError::InvalidRequest);
    }
...
pub async fn common_list(
    db: &SqlitePool,
    player_id: i64,
) -> Result<GetHeroGroupCommonListReply, AppError> {
    Ok(GetHeroGroupCommonListReply {
        hero_group_commons: hero_groups::get_hero_groups_common(db, player_id)
            .await?
            .into_iter()
            .map(Into::into)
            .collect(),
        hero_gourp_types: hero_groups::get_hero_group_types(db, player_id)
            .await?
            .into_iter()
            .map(Into::into)
            .collect(),
...
pub async fn current_list(
    db: &SqlitePool,
    player_id: i64,
) -> Result<GetHeroGroupListReply, AppError> {
    Ok(GetHeroGroupListReply {
        group_info_list: hero_groups::get_current_hero_group(db, player_id)
            .await?
            .into_iter()
            .map(Into::into)
            .collect(),
...
pub async fn change_selection(
    ...
    if type_id <= 0
        || current_select < 0
        || (current_select > 0
            && hero_groups::get_hero_group(db, player_id, current_select)
                .await?
                .is_none())
    {
        return Err(AppError::InvalidRequest);
    ...
    hero_groups::set_current_selection(db, player_id, type_id, current_select).await?;
...
pub async fn set_equip(
    ...
    hero_groups::set_hero_group_equip(db, player_id, group_id, index, equip_uids.clone()).await?;
