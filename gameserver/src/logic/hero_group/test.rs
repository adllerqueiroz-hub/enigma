use super::{
    check_name, delete_group, hero_group_snapshots, model,
    snapshot::{overlay_snapshot_catalog, snapshot_group},
    update_group, update_name, update_sort,
};
...
fn hero_group_name_uses_configured_character_limit() {
    ...
    assert!(check_name("队伍一").is_ok());
    assert!(check_name("").is_err());
    assert!(check_name("12345678901").is_err());
}
...
async fn common_group_rename_keeps_snapshot_in_sync() {
    ...
    update_name(&pool, 12, COMMON_SNAPSHOT_ID, 1, "Alpha".into())
        .await
        .unwrap();
...
async fn common_preset_sort_and_delete_share_one_persisted_order() {
    ...
    assert_eq!(
        update_sort(&pool, 15, COMMON_SNAPSHOT_ID, vec![2, 1])
            .await
            .unwrap()
            .sort_sub_ids,
        vec![2, 1]
    ...
    assert!(
        update_sort(&pool, 15, COMMON_SNAPSHOT_ID, vec![2, 2])
            .await
            .is_err()
    );
    assert_eq!(
        delete_group(&pool, 15, COMMON_SNAPSHOT_ID, 1)
            .await
            .unwrap()
            .sort_sub_ids,
        vec![2]
