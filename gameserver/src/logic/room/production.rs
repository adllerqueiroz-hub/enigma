...
pub async fn production_line_info(
    ...
    Ok(ProductionLineInfoReply {
        production_lines: room_ob::get_production_lines(db, player_id, ids)
            .await?
            .into_iter()
            .map(Into::into)
            .collect(),
...
pub async fn start_production_line(
    ...
    let production_line = room_ob::start_production_line_in_transaction(
        &mut tx, player_id, line_id, formula_id, count,
    )
    .await?;
    tx.commit().await?;
...
pub async fn gain_production_line(
    ...
    let before = room_ob::get_production_lines(db, player_id, ids).await?;
    if ids
        .iter()
    ...
    let mut tx = db.begin().await?;
    let changed = room_ob::gain_production_lines_in_transaction(&mut tx, player_id, &before)
        .await?
        .ok_or(AppError::InvalidRequest)?;
...
pub async fn production_line_lv_up(
    ...
    let current_level = room_ob::get_production_lines(db, player_id, &[line_id])
        .await?
        .into_iter()
...
pub async fn room_level_up(
    ...
    let current = block_packages::get_room_state(db, player_id)
        .await?
        .room_level;
    let next = current + 1;
    let Some(level) = tables.room_level.iter().find(|level| level.level == next) else {
        return Err(AppError::InvalidRequest);
    };
    let costs = scaled_formula_materials(&level.cost, 1);
    let mut tx = db.begin().await?;
    ...
    let production_lines =
        block_packages::level_up_room_in_transaction(&mut tx, player_id, current, next)
            .await?
            .ok_or(AppError::InvalidRequest)?
            .into_iter()
            .map(Into::into)
...
pub async fn production_line_accelerate(
    ...
    Ok(ProductionLineAccelerateReply {
        production_line: Some(
            room_ob::accelerate_production_line(db, player_id, line_id)
                .await?
                .into(),
        ),
    })
