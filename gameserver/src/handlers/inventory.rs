...
pub async fn on_get_currency_list(
    ...
    let reply = inventory::currency_list(ctx.state.db, player_id, msg.currency_ids).await?;
...
pub async fn on_exchange_same_currency(
    ...
    let reply = inventory::exchange_same_currency(ctx.state.db, player_id).await?;
...
pub async fn on_pop_exchange_same_currency(
    ...
    let reply =
        inventory::pop_exchange_same_currency(ctx.state.db, player_id, msg.currency_ids).await?;
...
pub async fn on_exchange_diamond(
    ...
    let reply = inventory::exchange_diamond(
        ctx.state.db,
        player_id,
        amount,
...
pub async fn on_get_item_list(
    ...
    let player_id = ctx.player()?.id;
    let reply = inventory::item_list(ctx.state.db, player_id).await?;
...
pub async fn on_get_buy_power_info(
    ...
    let reply = inventory::buy_power_info(ctx.state.db, player_id).await?;
...
pub async fn on_buy_power(ctx: &mut ConnectionContext, req: ClientPacket) -> Result<(), AppError> {
    ...
    let (reply, cost) = inventory::buy_power(ctx.state.db, player_id).await?;
...
pub async fn on_auto_use_expire_power_item(
    ...
    let player_id = ctx.player()?.id;
    let reply = inventory::auto_use_expired_power_items(ctx.state.db, player_id).await?;
...
pub async fn on_use_power_item(
    ...
    let (reply, power_items) = inventory::use_power_item(
        ctx.state.db,
        player_id,
        msg.uid.ok_or(AppError::InvalidRequest)?,
    )
    .await?;
...
pub async fn on_use_power_item_list(
    ...
    let (reply, power_items) =
        inventory::use_power_item_list(ctx.state.db, player_id, msg.use_power_item_info).await?;
...
pub async fn on_use_item(ctx: &mut ConnectionContext, req: ClientPacket) -> Result<(), AppError> {
    ...
    let (reply, changed, consumed, material_changes) =
        inventory::use_items(ctx.state.db, player_id, msg.entry, msg.target_id).await?;
...
pub async fn on_use_insight_item(
    ...
    let (reply, item_id) =
        inventory::use_insight_item(ctx.state.db, player_id, uid, hero_id).await?;
...
pub async fn on_mark_read_sub_type21(
    ...
    let reply = inventory::mark_read_sub_type21(ctx.state.db, player_id, item_id).await?;
